...
function WheelCard({ wheel, onSpin, onRestart, onDelete, categoryColors }: {
  wheel: Wheel
  onSpin: () => void
  onRestart: () => void
  onDelete: () => void
  ...
  return (
    <div className="card-glass rounded-2xl p-5 space-y-3 animate-[pop-in_200ms_cubic-bezier(0.34,1.56,0.64,1)]">
      <div className="flex items-start justify-between gap-2">
        <div>
          <h2 className="font-display font-bold text-lg text-ink leading-tight">{wheel.name}</h2>
      ...
      <main className="max-w-lg mx-auto px-4 py-2 space-y-4">
        {wheels.length === 0 ? (
          <div className="text-center py-16 space-y-3">
            ...
            <p className="text-ink/50 text-sm">No wheels yet. Create one to get spinning!</p>
          </div>
        ) : (
          wheels.map((wheel) => (
            <WheelCard
              ...
              onSpin={() => navigate(`/wheels/${wheel.id}`)}
