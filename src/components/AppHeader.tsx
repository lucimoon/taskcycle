...
function getNavLinks(settings: Settings) {
  const links = [
  ...
  if (settings.rewardsMenuEnabled) {
...
export function AppHeader({ theme, onThemeToggle }: Props) {
  ...
  function isActive(path: string, exact: boolean) {
    if (exact)
      return location.pathname === "/" || location.pathname === "/taskcycle";
  ...
  return (
    <header className="bg-white/50 backdrop-blur-lg border-b border-white/60 shadow-sm sticky top-0 z-40 overflow-x-clip">
      <div className="px-4 lg:px-6 py-3 flex items-center justify-between gap-4">
        <button
          onClick={() => navigate("/taskcycle")}
          className="font-display font-bold text-xl text-ink tracking-tight shrink-0"
        >
          TaskCycle
        </button>
        ...
        <nav className="hidden lg:flex items-center gap-1 flex-1">
          {getNavLinks(settings).map(({ label, path, exact }) => (
            <button
              key={path}
