...
export function PriorityUrgencyPicker({ priority, onChange }: PriorityUrgencyPickerProps) {
  return (
    <div className="space-y-2">
      <span className="text-sm font-bold text-ink">Priority</span>
      <div className="flex gap-2">
        {([1, 2, 3, 4] as Priority[]).map((level) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-center leading-tight mt-0.5 font-medium">{PRIORITY_LABELS[level]}</span>
            </button>
          )
        })}
      </div>
